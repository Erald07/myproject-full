...
function Index4(){
    ...
    useEffect(() => {
        const getSubSubSubSubCate = async () => {
            const result = await fetch(`http://localhost:8000/api/categoria-prodotto/${getcate}/${getsubcate}/${getsubsubcate}/${getsubsubsubcate}?colore=${colore}&marche=${marche}&genere=${genere}&eta=${eta}&taglia=${taglia}&exclude_out_of_stock=${availability}&min_price=${min_price}&max_price=${max_price}&orderBy=${orderBy}`);
        ...
        getSubSubSubSubCate();
    },[getcate, getsubcate, getsubsubcate, getsubsubsubcate, colore, marche, genere, eta, taglia, availability, min_price, max_price, orderBy]);
